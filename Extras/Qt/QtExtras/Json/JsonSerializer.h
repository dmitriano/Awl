...
namespace qtil
{
    ...
    inline awl::String TypeToString(QJsonValue::Type t)
    {
        switch (t)
        {
        ...
        throw JsonException(awl::format() << _T("Wrong type value: ") << t << _T("."));
    }
    ...
    inline void EnsureType(const QJsonValue & jv, QJsonValue::Type t)
    {
        if (jv.type() != t)
        {
            throw JsonException(awl::format() << _T("Expected value type: ") << TypeToString(t) << _T(" actul value type: ") << TypeToString(jv.type()));
        }
    }
