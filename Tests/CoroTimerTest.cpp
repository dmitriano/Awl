...
namespace
{
    ...
    awl::coro::Task<int> test(const awl::testing::TestContext& context, awl::testing::ITimeQueue& time_queue)
    {
        ...
        auto w3 = awl::coro::coSpawn(nullptr, wait_n(context, time_queue, 3));
        context.logger->debug("test step 2\n");
        auto w2 = awl::coro::coSpawn(nullptr, wait_n(context, time_queue, 2));
        context.logger->debug("test step 3\n");
        auto w1 = awl::coro::coSpawn(nullptr, wait_n(context, time_queue, 1));
...
AWL_TEST(CoroJobStartsOnCoSpawn)
{
    ...
    job = awl::coro::coSpawn(nullptr, std::move(job));
...
AWL_TEST(CoroTaskStartsOnCoSpawn)
{
    ...
    task = awl::coro::coSpawn(nullptr, std::move(task));
...
AWL_EXAMPLE(CoroTimer)
{
    ...
    auto result = awl::coro::coSpawn(nullptr, test(context, time_queue));
...
AWL_EXAMPLE(CoroTimer0)
{
    ...
    auto result = awl::coro::coSpawn(nullptr, wait_0(context, time_queue));
